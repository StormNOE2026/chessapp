    ...
    return (
        <div className="auth-screen">
            <div className="auth-box">
                ...
                <div style={{
                    backgroundColor: 'rgba(245, 158, 11, 0.1)', color: '#fbbf24', padding: '12px',
                    borderRadius: '8px', border: '1px solid rgba(245, 158, 11, 0.3)', fontSize: '13px',
                ...
                <form className="auth-form" onSubmit={handleSubmit}>
                    <input className="auth-input" type="email" required placeholder="Email" value={email} onChange={(e) => setEmail(e.target.value)} />
                    <input className="auth-input" type="password" required placeholder="Password" value={password} onChange={(e) => setPassword(e.target.value)} />
                    <button className="auth-btn" disabled={loading} type="submit">{loading ? '...' : (isLogin ? 'Log In' : 'Sign Up')}</button>
                </form>
...
function ChessGame({ user, onLogout }) {
        ...
        channel
            ...
            .on('broadcast', { event: 'chat' }, ({ payload }) => {
                if (payload.targetEmail === userEmail && payload.senderEmail === opponentRef.current) {
                    ...
                    if (speakChatEnabledRef.current) speak(payload.text);
                }
            })
        ...
        if (displayGame.isCheckmate()) {
            ...
            if (!isGameOverManually) {
                ...
                if (gunshotEnabledRef.current) {
                    for (let i = 0; i < 3; i++) setTimeout(() => new Audio('/shotgun.mp3').play().catch(() => { }), i * 400);
                }
    ...
    const handleResign = () => {
        ...
        if (window.confirm("Are you sure you want to resign?")) {
            setIsGameOverManually(true);
            setStatus("You resigned. You Lose!");
            speak("You resigned. You Lose!");
            recordResult('loss');
    ...
    const handleDrawOffer = () => {
        ...
        if (opponent) {
            ...
            setIsGameOverManually(true);
            setStatus("Computer accepts the draw!");
            speak("Computer accepts the draw!");
            recordResult('draw');
        }
    };
    ...
    rowOrder.forEach((row) => {
        colOrder.forEach((col) => {
            ...
            board.push(
                <div key={square} className={squareClass} onClick={() => onSquareClick(square)}>
    ...
    // ==========================================
    return (
        <div style={{ display: 'flex', minHeight: '100vh', width: '100vw', backgroundColor: '#121212', overflow: 'hidden' }}>
            ...
            >
                <div style={{ display: 'flex', alignItems: 'center', padding: '15px 18px', borderBottom: '1px solid #333', marginBottom: '10px', height: '56px', flexShrink: 0 }}>
                ...
                {sideMenuItems.map((item) => (
                    <div
                        ...
                        onClick={() => {
                            if (item.label === 'Community') {
                                ...
                                setActiveTab('panels'); // Switch to panels on mobile
                        ...
                        style={{
                            display: 'flex', alignItems: 'center', padding: '12px 18px', cursor: 'pointer',
                            whiteSpace: 'nowrap', transition: 'background 0.2s', color: '#b0b0b0'
                        }}
                    ...
                    >
                        ...
                        <span style={{
                            marginLeft: '10px', fontSize: '15px', fontWeight: 'bold',
                            opacity: isSidebarHovered ? 1 : 0, transition: 'opacity 0.2s'
                        }}>{item.label}</span>
                    </div>
                ))}
            </nav>
            ...
            {/* 2. MAIN APP ROOT */}
            <div className="app-root" style={{ flexGrow: 1, width: 'calc(100vw - 60px)' }}>

                {/* HEADER */}
                <header className="app-header">
                    <h2 className="app-header-logo">ChessOnline</h2>
                ...
                <div className="notification-bar">
                    {incomingChallenge && (
                        <div className="challenge-banner">
                            ...
                            <button className="banner-btn btn-accept" onClick={handleAcceptChallenge}>Accept</button>
                            <button className="banner-btn btn-decline" onClick={handleDeclineChallenge}>Decline</button>
                        </div>
                    )}
                    {incomingDrawOffer && (
                        <div className="draw-banner">
                            <span>🤝 Opponent offered a Draw!</span>
                            <button className="banner-btn btn-accept" onClick={acceptDraw}>Accept</button>
                            <button className="banner-btn btn-decline" onClick={handleDeclineDraw}>Decline</button>
                        </div>
                    )}
                </div>
                ...
                <div className="game-layout">
                    ...
                    {/* BOARD COLUMN */}
                    <div className={`col-board ${activeTab === 'board' ? 'tab-active' : ''}`} style={{ width: '100%', maxWidth: '560px' }}>
                        ...
                        <div className="chess-board" style={{ width: '100%', aspectRatio: '1/1' }}>
                            {board}
                        </div>

                        <div className="controls-row" style={{ width: '100%', marginTop: '10px' }}>
                            <button className="ctrl-btn" onClick={handleDrawOffer} disabled={(!opponent && !isPlayingComputer) || isGameOverManually}>🤝 Draw</button>
                            <button className="ctrl-btn" onClick={handleResign} disabled={(!opponent && !isPlayingComputer) || isGameOverManually}>🏳️ Resign</button>
                        </div>
                    </div>

                    {/* MIDDLE PANELS COLUMN */}
                    <div className={`col-panels ${activeTab === 'panels' ? 'tab-active' : ''}`}>

                        {/* Stats Card */}
                        <div className="panel-card">
                            <div className="panel-card-header">Your Stats</div>
                            <div className="panel-card-body stats-row">
                                <div className="stat-item">
                                    <span className="stat-key win">WON</span>
                                    <span className="stat-value">{stats.wins}</span>
                                </div>
                                <div className="stat-item">
                                    <span className="stat-key loss">LOSS</span>
                                    <span className="stat-value">{stats.losses}</span>
                                </div>
                                <div className="stat-item">
                                    <span className="stat-key draw">DRAW</span>
                                    <span className="stat-value">{stats.draws}</span>
                                </div>
                            </div>
                        </div>

                        {/* Optional Community Chat Panel */}
                        {showCommunityChat && (
                            <div className="panel-card chat-box">
                                <div className="panel-card-header" style={{ color: '#f97316' }}>🌍 Community Chat</div>
                                ...
                                <form className="chat-form" onSubmit={sendCommunityMessage}>
                                    <input id="community-input" className="chat-input" type="text" value={communityInput} onChange={e => setCommunityInput(e.target.value)} placeholder="Say something..." />
                                    <button className="chat-send" style={{ backgroundColor: '#f97316', color: 'white' }} type="submit">Send</button>
                                </form>
                            </div>
                        )}
                            ...
                            <div className="panel-card-body players-list">
                                {viewMode === 'online' && (
                                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px', fontSize: '11px' }}>
                                        ...
                                        <select className="view-select" value={challengeTime} onChange={(e) => setChallengeTime(Number(e.target.value))}>
                                ...
                                {viewMode === 'online' && onlineUsers.map((u, i) => (
                                    <div className="player-row" key={i}>
                                        ...
                                        {u.email !== userEmail && <button className="challenge-btn" onClick={() => handleSendChallenge(u.email)}>Challenge</button>}
                                    </div>
                                ))}
                                {viewMode === 'all' && allMembers.map((u, i) => (
                                    <div className="player-row" key={i}><span>{u.email.split('@')[0]}</span></div>
                                ))}
                                {viewMode === 'tv' && tvGames.map((game, i) => (
                                    <a className="tv-card" key={i} href={game.url} target="_blank" rel="noreferrer">
                                ...
                                {viewMode === 'chesscom' && chessComStreamers.map((streamer, i) => (
                                    <a className="streamer-card" key={i} href={streamer.twitch_url} target="_blank" rel="noreferrer">
                                        <img className="streamer-avatar" src={streamer.avatar} alt={streamer.username} />
                                        <div>
                        ...
                        <button className="play-computer-btn" onClick={() => {
                            setOpponent(null);
                        ...
                        <div className="controls-row">
                            <button className="ctrl-btn" onClick={() => setGunshotEnabled(!gunshotEnabled)}>
                                {gunshotEnabled ? '🔇 Gunshot Off' : '🔊 Gunshot On'}
                            </button>
                            <button className="ctrl-btn" onClick={() => setSpeakChatEnabled(!speakChatEnabled)}>
                                {speakChatEnabled ? '🔇 Chat Speak Off' : '🔊 Chat Speak On'}
                            </button>
                        </div>
                        ...
                        <div className="panel-card chat-box">
                            ...
                            <div className="chat-messages">
                                {chatMessages.map((m, i) => (
                                    <div className={`chat-msg ${m.sender === userEmail ? 'mine' : 'theirs'}`} key={i}>
                            ...
                            <form className="chat-form" onSubmit={sendChatMessage}>
                                <input className="chat-input" disabled={!opponent} type="text" value={chatInput} onChange={(e) => setChatInput(e.target.value)} placeholder={opponent ? "Type message..." : "Chat locked"} />
                                <button className="chat-send" type="submit">Send</button>
                            </form>
                        </div>

                        {/* Move History */}
                        <div className="panel-card" style={{ display: 'flex', flexDirection: 'column', height: '180px' }}>
                            ...
                            <div className="history-nav">
                                <button className="nav-btn" onClick={() => setCurrentMoveIndex(0)}>⏪</button>
                                <button className="nav-btn" onClick={() => setCurrentMoveIndex(prev => Math.max(0, prev - 1))}>◀️</button>
                                <button className="nav-btn" onClick={() => setCurrentMoveIndex(prev => Math.min(moveHistory.length, prev + 1))}>▶️</button>
                                <button className="nav-btn" onClick={() => setCurrentMoveIndex(moveHistory.length)}>⏩</button>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className={`col-ads ${activeTab === 'ads' ? 'tab-active' : ''}`}>
                        <div className="ads-header">✈️ TRAVEL DEALS</div>
                        <div className="ads-scroll">
                            {travelAds.map(ad => (
                                <a className="ad-card" key={ad.id} href={ad.url} target="_blank" rel="noreferrer">
                                    <img className="ad-img" src={ad.img} alt="Destination" loading="lazy" />
